...
export default function Index() {
  ...
  const [outreachCandidate, setOutreachCandidate] = useState<ScoredCandidate | null>(null);
...
${recruiterName || "NEKTAB"}`;
  ...
  if (sessionLoading || recoveryMode || !session) {
    return (
      <AuthOverlay
        session={session}
  ...
  return (
    <div className="site-shell min-h-screen bg-[#fafafa]">
          ...
          <section className="site-section relative -mt-10 bg-[#f5f5f2] pt-20 pb-20">
            <div className={`container grid gap-7 ${showSidePanel ? "lg:grid-cols-[330px_minmax(0,1fr)]" : "lg:grid-cols-1"}`}>
              ...
              <div className="min-h-0 space-y-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                    ...
                    {selectedCandidates.length >= 2 && (
                      <Button
                        ...
                        className="rounded-full bg-primary text-black font-bold normal-case h-9 hover:bg-primary/95 flex items-center gap-1.5 shadow-[0_0_15px_rgba(95,200,145,0.25)] border border-primary/20"
                      >
                        <Award className="h-4 w-4" />
                        Jämför valda ({selectedCandidates.length})
                      </Button>
                    )}
                    <Button
                      type="button"
                ...
                <AnalyticsDashboard
                  candidatePool={filteredWebResults}
                  requirements={requirements}
                />
      ...
      <CompareCandidatesModal
        ...
        candidates={filteredWebResults.filter(c => selectedCandidateIds.has(candidateId(c)))}
      />

      <OutreachEditor
        isOpen={outreachCandidate !== null}
